...
export default async function Cuenta({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {hasShop && <TimeSeries title="Ventas netas diarias (Shopify)" unit="MXN" points={netSales} markers={markers} fmt="mxn0" />}
      {hasShop && <TimeSeries title="MER diario" unit={sharedSpend ? "ventas netas ÷ gasto Meta de todas las cuentas de la tienda" : "ventas netas ÷ gasto Meta"} points={merPts} markers={markers} fmt="fixed1" />}
      <TimeSeries title="ROAS diario (Meta)" points={roas} markers={markers} fmt="fixed1" />
      <TimeSeries title="CPA diario (Meta)" unit="MXN por compra" points={cpa} markers={markers} fmt="mxn0" />
