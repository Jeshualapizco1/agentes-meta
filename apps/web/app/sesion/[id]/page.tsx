  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        ...
        <h1 className="font-serif text-2xl font-medium">{s.actor_name}: {s.summary}</h1>
      ...
      <section className="rounded-md border border-line bg-surface p-4">
        ...
        <form action={annotate} className="mt-3 grid gap-2 sm:grid-cols-2">
          ...
          <button className="w-fit rounded bg-accent px-3 py-1.5 text-sm font-semibold text-white">Guardar razón</button>
        </form>
      </section>

      <section className="flex flex-col gap-3">
        <h2 className="font-serif text-xl">Objetos tocados</h2>
        {(groups ?? []).map(g => (
          <details key={g.id} className="rounded-md border border-line bg-surface">
