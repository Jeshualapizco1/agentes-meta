...
function Field({ name, label, help, value, unit, step = "any" }: { name: string; label: string; help: string; value: number | null | undefined; unit?: string; step?: string }) {
  return (
    <label className="flex flex-col gap-1">
      ...
      <input name={name} type="number" step={step} defaultValue={value ?? ""} className="tnum rounded border border-line bg-paper px-3 py-2 text-sm" />
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-wrap items-end gap-4">
        <div><p className="font-mono text-[11px] uppercase tracking-wider text-muted">Configuración por cuenta</p><h1 className="font-serif text-3xl font-medium">Con qué números decide el agente</h1></div>
        <form method="get" className="ml-auto flex gap-2"><select name="account" defaultValue={accountId} className="rounded border border-line bg-surface px-2 py-1 text-sm">{(accounts ?? []).map(a => <option key={a.id} value={a.id}>{a.name}</option>)}</select><button className="rounded bg-accent px-3 py-1 text-sm font-semibold text-white">Cambiar</button></form>
      </div>
      {p.saved && <p className="rounded bg-accent-soft px-3 py-2 text-sm text-accent">Guardado. Los agentes usan estos valores desde la próxima corrida.</p>}
      ...
      <form action={saveProfile} className="flex flex-col gap-6">
        ...
        <section className="rounded-md border border-line bg-surface p-5">
          <h2 className="mb-1 font-serif text-xl">Economía</h2>
        ...
        <section className="rounded-md border border-line bg-surface p-5">
          <h2 className="mb-1 font-serif text-xl">Límites de gasto y candados</h2>
        ...
        <section className="rounded-md border border-line bg-surface p-5">
          <h2 className="mb-1 font-serif text-xl">Modo y campañas permitidas</h2>
        ...
        <section className="rounded-md border border-line bg-surface p-5">
          <h2 className="mb-1 font-serif text-xl">Noes duros</h2>
          ...
          <textarea name="hard_noes" rows={4} defaultValue={prof?.hard_noes ?? ""} className="w-full rounded border border-line bg-paper px-3 py-2 text-sm" />
        </section>
        <div className="flex items-center gap-4"><button className="rounded bg-accent px-4 py-2 text-sm font-semibold text-white">Guardar configuración</button>{prof?.updated_at && <span className="font-mono text-[11px] text-muted">última actualización {fmtDay(prof.updated_at)} {fmtTime(prof.updated_at)}</span>}</div>
      </form>

      {history?.length ? <section className="rounded-md border border-line bg-surface p-5"><h2 className="mb-2 font-semibold">Últimos cambios de configuración</h2><ul className="text-sm">{history.map((h, i) => <li key={i} className="border-t border-line py-1 first:border-t-0"><span className="font-mono text-[11px] text-muted">{fmtDay(h.created_at)} {fmtTime(h.created_at)}</span> · <b>{String(h.changed_by).split("@")[0]}</b> · ROAS obj. {h.patch?.target_roas ?? "—"} · CPA obj. {h.patch?.target_cpa ?? "—"} · techo {h.patch?.daily_spend_ceiling ?? "—"} · modo {h.patch?.mode}</li>)}</ul></section> : null}
    </div>
  );
}
