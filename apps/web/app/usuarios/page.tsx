...
export default async function Usuarios({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {p.error && <p className="rounded-xl bg-crit-soft px-3 py-2 text-sm text-crit">{decodeURIComponent(p.error)}</p>}
      <Card>
        ...
        <form action={createUser} className="grid gap-2 sm:grid-cols-5">
          ...
          <input name="password" type="text" required minLength={8} placeholder="contraseña (mín. 8)" className="rounded-lg border border-line bg-paper px-3 py-2 text-sm" />
          ...
          <button className="btn-accent px-3 py-2 text-sm font-semibold text-white">Guardar</button>
        </form>
      </Card>
      <Card>
