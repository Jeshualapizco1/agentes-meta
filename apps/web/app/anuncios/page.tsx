        ...
        <div className="overflow-x-auto"><table className="w-full text-[13px]"><thead><tr className="text-left font-mono text-[11px] uppercase text-muted"><th className="px-4 py-2">Anuncio</th><th>Estado</th><th className="text-right">Gasto</th><th className="text-right">Compras</th><th className="text-right">Valor</th><th className="text-right">ROAS</th><th className="text-right">CPA</th><th className="text-right">CTR</th><th>Revisión</th><th></th></tr></thead><tbody>
          {list.map(a => (
            <tr key={a.id} className={`tnum border-t border-line align-middle ${a.active ? "" : "text-muted"}`}>
              ...
              <td><Chip tone={a.active ? "ok" : "neutral"}>{a.active ? "activo" : (a.effective_status ?? "—").toLowerCase()}</Chip></td>
              <td className="text-right">{mxn0(a.spend)}</td><td className="text-right">{a.purchases.toFixed(0)}</td><td className="text-right">{mxn0(a.value)}</td>
              <td className="text-right">{a.roas != null ? a.roas.toFixed(2) : "—"}</td><td className="text-right">{a.cpa != null ? mxn0(a.cpa) : "—"}</td><td className="text-right">{a.ctr != null ? a.ctr.toFixed(2) + "%" : "—"}</td>
              <td>{a.review ? <span className="flex items-center gap-2 font-mono text-[11px] text-muted"><span className="flex h-6 w-6 items-center justify-center rounded-full text-[10px] font-bold text-white" style={{ background: "var(--gradient-accent)" }} title={a.review.reviewed_by}>{initials(a.review.reviewed_by.split("@")[0]!.replace(".", " "))}</span>{fmtDay(a.review.created_at).split(",")[0]} {fmtTime(a.review.created_at)}{a.review.note && <span title={a.review.note}> · nota</span>}</span> : <span className="font-mono text-[11px] text-muted">nunca</span>}{a.unreviewed && <Chip tone="amber">sin revisar</Chip>}</td>
