...
export const metadata = { title: "Detalle de sesión" };

export default async function Sesion({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  return (
    <div className="flex flex-col gap-6">
      <a href={sessionReturnPath(p.returnTo, s.account_id)} className="ui-button ui-button-ghost w-fit">← Volver a la vista anterior</a>
      <div>
      ...
      <Card>
        ...
        <form action={annotate} className="mt-3 grid gap-2 sm:grid-cols-2">
          ...
          <button className="w-fit btn-accent px-3 py-1.5 text-sm font-semibold text-on-accent">Guardar razón</button>
        </form>
      </Card>
