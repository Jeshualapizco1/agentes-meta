...
export default async function Horarios({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  const accountId = p.account ?? "1703313583465547"; const metric = (p.metric ?? "roas") as "roas" | "spend" | "purchases" | "cpa"; const range = readRange(p.weeks && !p.days && !p.from && !p.to ? { days: String(Number(p.weeks) * 7) } : p, 28);
  if (!range) return <InvalidPeriod path="/horarios" account={p.account} />;
    ...
    <div className="flex flex-col gap-6">
      <div className="flex flex-wrap items-end gap-4">
        ...
        <form key={JSON.stringify(p)} method="get" className="ml-auto flex flex-wrap items-end gap-2">
          ...
          <DateRange days={range.days} from={p.from} to={p.to} presets={[14, 28, 56]} label="Periodo" />
          <button className="btn-accent px-3 py-1 text-sm font-semibold text-on-accent">Ver</button>
        </form>
      </div>
