...
export default async function Cuenta({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  const range = readRange(p, 30);
  if (!range) return <InvalidPeriod path="/cuenta" account={p.account} />;
  ...
  const markers: Marker[] = (sessions ?? []).map(s => ({ id: s.id, date: CDMX_DAY.format(new Date(s.started_at)), time: fmtTime(s.started_at), actor: s.actor_name, summary: s.summary, resets: s.resets_learning, href: sessionHref(s.id, accountId, pageHref("/cuenta", p)) }));
