...
export default async function Hoy({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  const [{ data: prof }, { data: rows }, { data: shopRows }, { data: sessions }, { data: alerts }] = await Promise.all([
    ...
    acc?.shopify_domain ? sb.from("shopify_daily").select("date,net_sales,new_customers,is_closed_day").eq("account_id", accountId).gte("date", sinceDate).order("date") : Promise.resolve({ data: [] as { date: string; net_sales: number; new_customers: number; is_closed_day: boolean }[] }),
  ...
  // Agregado diario (misma regla que Cuenta): días cerrados en la zona de la cuenta; gasto de las cuentas hermanas solo para el MER
  ...
  const shop = new Map((shopRows ?? []).map(s => [s.date, { net: Number(s.net_sales ?? 0), newc: s.new_customers ?? 0, closed: s.is_closed_day }]));
  ...
  const s7 = sum(last7, "spend"), sp7 = sum(prev7, "spend"), v7 = sum(last7, "value"), vp7 = sum(prev7, "value"), c7 = sum(last7, "purchases"), cp7 = sum(prev7, "purchases"), sa7 = sum(last7, "spendAll"), sap7 = sum(prev7, "spendAll");
  const roas7 = ratio(v7, s7), roasP = ratio(vp7, sp7), cpa7 = ratio(s7, c7), cpaP = ratio(sp7, cp7);
  const shopClosed = last7.filter(d => shop.get(d)?.closed), shopPrev = prev7.filter(d => shop.get(d)?.closed);
  const sumS = (ds: string[], k: "net" | "newc") => ds.reduce((n, d) => n + (shop.get(d)?.[k] ?? 0), 0);
  const spendOf = (ds: string[]) => ds.reduce((n, d) => n + byDate.get(d)!.spendAll, 0);
  const mer7 = shopClosed.length ? ratio(sumS(shopClosed, "net"), spendOf(shopClosed)) : null, merP = shopPrev.length ? ratio(sumS(shopPrev, "net"), spendOf(shopPrev)) : null;
  const cac7 = shopClosed.length ? ratio(spendOf(shopClosed), sumS(shopClosed, "newc")) : null, cacP = shopPrev.length ? ratio(spendOf(shopPrev), sumS(shopPrev, "newc")) : null;
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-12 lg:auto-rows-min">
        ...
        <Card span={4} eyebrow="MER · ventas netas ÷ gasto Meta (Shopify)">
          {hasShop ? (
            <div className="flex flex-col gap-5">
              <Kpi label="" value={mer7} prev={merP} format={v => v.toFixed(2)} hint={siblings.length > 1 ? "gasto de todas las cuentas de la tienda" : undefined} />
              <Kpi label="CAC · gasto ÷ clientes nuevos" value={cac7} prev={cacP} format={mxn0} higherIsBetter={false} />
            </div>
          ) : (
            <div className="rounded-xl bg-amber-soft px-4 py-3 text-sm"><b>Shopify todavía sin datos.</b> {acc?.shopify_domain ? `La cuenta está ligada a ${acc.shopify_domain}; falta el token SHOPIFY_ADMIN_TOKEN (docs/02-accesos.md).` : "Esta cuenta no tiene tienda ligada."}</div>
          )}
        </Card>
        ...
        <Card span={4} eyebrow="CPA · últimos 7 días cerrados (Meta)">
          <Kpi label="" value={cpa7} prev={cpaP} format={mxn0} higherIsBetter={false} target={prof?.target_cpa ? { value: Number(prof.target_cpa), label: mxn0(Number(prof.target_cpa)) } : undefined} hint={`${c7.toFixed(0)} compras · gasto ${mxn0(s7)}`} />
        </Card>
