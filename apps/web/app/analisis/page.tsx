...
type Win = { session_id: string; horizon: string; status: string; confidence: string | null; verdict: string | null; delta: { roas_pct: number | null; control_roas_pct: number | null; cpa_pct: number | null } | null };
...
export default async function Analisis({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  const { data: wins } = ids.length ? await sb.from("evaluation_windows").select("session_id,horizon,status,confidence,verdict,delta").in("session_id", ids) : { data: [] as Win[] };
      ...
      <Card eyebrow="Sesiones de cambios · últimos 30 días" title="Veredicto por sesión (72 h · 7 días · 14 días)">
        {sessions?.length ? <ul className="flex flex-col divide-y divide-line">{sessions.map(s => { const ws = (byS.get(s.id) ?? []).sort((a, b) => ["72h", "7d", "14d"].indexOf(a.horizon) - ["72h", "7d", "14d"].indexOf(b.horizon)); return (
          <li key={s.id} className="flex flex-col gap-2 py-3">
            ...
            {ws.length ? <div className="grid gap-2 sm:grid-cols-3">{ws.map(w => <div key={w.horizon} className="rounded-xl border border-line bg-paper px-3 py-2 text-[12px]"><p className="mb-1 flex items-center gap-2"><Chip tone={tone(w)}>{H_LABEL[w.horizon]}</Chip><span className="font-mono text-[11px] text-muted">{w.status === "mature" ? "maduro" : w.status === "preliminary" ? "preliminar" : "pendiente"} · {CONF[w.confidence ?? "insufficient"]}</span></p><p className="leading-snug">{w.verdict}</p></div>)}</div> : <p className="font-mono text-[11px] text-muted">sin ventanas calculadas todavía (corre el analista)</p>}
