...
function Evaluation({ preview }: { preview: DecisionFormResult["preview"] }) {
  ...
  return <details className="rounded-xl border p-4"><summary className="cursor-pointer text-sm">Ver evaluación · {preview.proposals.filter(p => !p.blocked).length} aptos · {preview.proposals.filter(p => p.blocked).length} bloqueados · {preview.exclusions.length} exclusiones</summary>
    <p className="my-3 text-xs text-muted">Lectura: {momentLabel(preview.evaluatedAt)}. Evaluar no guarda propuestas ni cambia Meta.</p>
    ...
    {preview.exclusions.length > 0 && <details className="mt-4"><summary>Por qué otras entidades no generan propuestas</summary><ul className="mt-2 space-y-2 text-xs text-muted">{preview.exclusions.map((e, i) => <li key={i}>{e.entityId ? `${preview.proposals.find(p => p.entity_id === e.entityId)?.entity_name ?? "Una campaña"}: ` : "Regla: "}{e.reason}</li>)}</ul></details>}
  </details>;
}
function Opportunity({ opportunity: o, account, currency }: { opportunity: DecisionOpportunity; account: string; currency: string }) {
  ...
  return <article className="rounded-2xl border bg-surface p-5">
    ...
    <h3 className="mt-3 break-words text-lg font-semibold">{o.entityName}</h3><p className="mt-1 text-sm text-muted">{o.title}</p>
    ...
    <details className="text-xs text-muted"><summary className="cursor-pointer">Ver evidencia · {o.reading.available}/7 días</summary><p className="mt-2">{o.explanation}</p><p className="mt-2">{shortDate(o.reading.from)} a {shortDate(o.reading.to)} · Actualización más antigua: {momentLabel(o.reading.fetchedAt)}.</p></details>

    <div className="my-4 flex flex-wrap gap-2"><a className="ui-button ui-button-primary" href={`/experimentos?${new URLSearchParams({ account, campaign: o.entityId, nuevo: "1" })}`}>Preparar prueba →</a><a className="ui-button ui-button-secondary" href={`/anuncios?account=${account}&from=${o.reading.from}&to=${o.reading.to}&campaign=${o.entityId}`}>Comparar anuncios</a></div>
...
export function DecisionDesk({ account, accounts, admin, profile, opportunities, rules, reviews, evaluation, today }: Props) {
  ...
  return <div className="space-y-6">
    <header className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs uppercase tracking-widest text-muted">DECIDIR QUÉ PROBAR</p><h1 className="mt-2 text-3xl font-semibold">Dónde enfocar la próxima prueba</h1><p className="mt-2 text-sm text-muted">{account.name} · días cerrados anteriores al {shortDate(today)}</p></div><form action="/decisiones" method="get" className="flex flex-wrap gap-2"><label className="sr-only" htmlFor="decision-account">Cuenta</label><select id="decision-account" name="account" defaultValue={account.id} className="max-w-full border p-2">{accounts.map(a => <option key={a.id} value={a.id}>{a.name}</option>)}</select><Button type="submit">Cambiar</Button></form></header>
    <div className="flex flex-wrap items-center gap-3 text-sm"><Chip tone="amber">{protect} para revisar gasto</Chip><Chip tone="meta">{scale} con potencial</Chip><a className="ml-auto underline text-muted" href={`/experimentos?account=${account.id}`}>Ver mis pruebas →</a></div>
    <section aria-labelledby="opportunity-title"><h2 id="opportunity-title" className="mb-4 text-xl font-semibold">Campañas para revisar</h2>{opportunities.length ? <div className="grid gap-4 xl:grid-cols-2">{opportunities.map(o => <Opportunity key={o.id} opportunity={o} account={account.id} currency={account.currency} />)}</div> : <p className="rounded-xl border p-5 text-muted">No hay campañas activas con inversión en esta ventana. Consulta Rendimiento para revisar los datos.</p>}</section>
    <details className="rounded-2xl border bg-surface p-5"><summary className="cursor-pointer font-semibold">Avanzado · reglas y propuestas del agente</summary><p className="mt-3 text-sm text-muted">{profile.mode === "off" ? "Agente apagado" : profile.mode === "semi" ? "El agente propone, tú apruebas" : profile.mode === "auto" ? "Automático" : "Modo sin verificar"} · {profile.dry_run ? "Nada se envía a Meta (simulación)" : "Ejecución real habilitada"}. <a className="underline" href={`/configuracion?account=${account.id}`}>Ver límites</a></p>{profile.hard_noes && <p className="mt-3 text-sm">Restricciones: {profile.hard_noes}</p>}<section className="mt-5" aria-labelledby="rules-title"><h2 id="rules-title" className="text-xl font-semibold">Reglas para proponer cambios</h2><p className="my-3 text-sm text-muted">{rules.filter(r => r.status === "activa" && r.condition?.version === 1).length} reglas estructuradas activas. Evaluar es de solo lectura; guardar requiere administrador.</p>
      {rules.map(r => <details key={`${r.id}:${r.version}`} className="my-3 rounded-xl border p-4"><summary className="cursor-pointer">{r.name} · {r.status === "activa" ? "Activa" : "Inactiva"} · revisión {r.version}</summary>{r.condition?.version === 1 ? <PolicyEditor account={account.id} rule={r} admin={admin} maxPct={profile.max_budget_change_pct} /> : <p className="mt-3 text-sm text-muted">Regla heredada sin criterios estructurados. No produce candidatos con este motor. Crea una regla revisada abajo.</p>}</details>)}
      <details className="mt-4 rounded-xl border p-4"><summary className="cursor-pointer">Preparar una regla nueva</summary><PolicyEditor account={account.id} admin={admin} maxPct={profile.max_budget_change_pct} /></details>
    </section>
    <section className="space-y-4 rounded-2xl border bg-surface p-5"><h2 className="text-xl font-semibold">Propuestas para revisar</h2><p className="text-sm text-muted">El agente evalúa reglas activas, evidencia reciente y límites. Crear la cola no equivale a aprobarla.</p><form action={generate}><input type="hidden" name="account" value={account.id} /><Button type="submit" disabled={pending}>{pending ? "Evaluando…" : "Generar propuestas ahora"}</Button></form><Feedback result={result} /><Evaluation preview={result.preview ?? evaluation} /><a href={`/hoy?account=${account.id}`} className="ui-button ui-button-secondary">Revisar propuestas en Hoy</a></section></details>
    <details className="rounded-2xl border bg-surface p-5"><summary className="cursor-pointer font-semibold">Decisiones registradas ({reviews.length})</summary>{reviews.length ? <ul className="mt-4 space-y-4">{reviews.map(r => <li key={r.id} className="border-t pt-3 text-sm"><p>{opportunities.find(o => o.entityId === r.entity_id)?.entityName ?? r.entity_id} · {r.status === "approved" ? "Investigar" : "No intervenir"}</p><p className="my-1">{r.rationale}</p><p className="text-xs text-muted">{r.decided_by} · {momentLabel(r.decided_at)} · sin acción en Meta</p></li>)}</ul> : <p className="mt-3 text-sm text-muted">Aún no has registrado decisiones para estas oportunidades.</p>}</details>
  </div>;
}
