...
export function ProposalDetails({ proposal, currency }: { proposal: HoyProposal; currency: string }) {
  ...
  return <>
    <div className="flex flex-wrap items-center gap-2"><Chip tone="amber">Pendiente de revisión</Chip><span className="text-xs text-muted">{proposal.entityLevel}</span></div><p className="mt-4 text-lg font-semibold hoy-wrap">{proposal.entity}</p><p className="mt-2 text-xs text-muted">Regla: {proposal.rule}</p>
    <section className="hoy-detail-section"><h3>Qué cambiaría</h3>
      ...
      <p className="mt-4 text-sm text-muted">{change.kind === "pause" ? "En una ejecución real, se solicitaría pausar este anuncio; no la campaña completa." : change.kind === "budget" ? "En una ejecución real, se solicitaría cambiar el presupuesto diario de esta entidad. No es una garantía de gasto ni de resultado." : "Un movimiento real requiere verificar las dos campañas y recuperar el par si falla una parte. No se ejecuta en este piloto."}</p>
    </section>
    ...
    <section className="hoy-detail-section"><h3>Candados al crear la propuesta</h3><p className="mb-4 text-xs text-muted">Son una fotografía del momento de evaluación. Deben verificarse otra vez antes de una ejecución.</p>
