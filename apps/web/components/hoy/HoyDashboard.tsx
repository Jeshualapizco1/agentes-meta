...
import { CampaignReadings } from "./CampaignReadings";
...
export function HoyDashboard({ snapshot, accountControl, onReview, onControl, targetRoas }: {
  ...
  const quiet = snapshot.proposals.state === "ready" && snapshot.alerts.state === "ready" && !proposals.length && !alerts.length;
  ...
  return <div className="hoy-page">
    ...
    <div className="hoy-context"><div className="hoy-context-account"><span className="hoy-account-mark" aria-hidden="true">{account.name.slice(0, 1)}</span><div><p className="font-semibold hoy-wrap">{account.name}</p><p className="text-xs text-muted">{account.currency}</p></div></div>
      <div className="hoy-mode-controls"><Chip tone={agent.execution === "simulation" ? "meta" : "amber"}>{agent.execution === "simulation" ? "Simulación: nada se envía a Meta" : agent.execution === "live" ? "Ejecución real habilitada" : "Modo sin verificar"}</Chip><Button variant={agent.brake === "engaged" ? "danger" : "secondary"} onClick={onControl}>{agent.brake === "engaged" ? "Ver freno activo" : "Control del agente"}</Button></div>
    </div>
    ...
    <CampaignReadings snapshot={snapshot} />
    ...
    {!!snapshot.evaluatingExperiments && <p className="rounded-xl border border-amber bg-amber-soft p-3 text-sm"><a className="underline" href={`/experimentos?account=${account.id}`}>{snapshot.evaluatingExperiments} pruebas esperan tu veredicto</a></p>}
    <div className="hoy-bento">
      <Card hero className={quiet ? "hoy-attention hoy-attention-quiet" : "hoy-attention"} eyebrow="TU SIGUIENTE DECISIÓN" title="Necesita tu atención" action={<Chip tone="neutral">{quiet ? "No hay propuestas pendientes" : snapshot.proposals.state === "ready" ? `${proposals.length} propuestas` : "Cola sin verificar"}</Chip>}>
        {quiet ? <p className="flex flex-wrap justify-between gap-3 text-sm">Sin pendientes del agente <a className="text-meta underline" href={`/decisiones?account=${account.id}`}>Ver Decisiones</a></p> : <>
        <section className="hoy-alerts" aria-label="Alertas prioritarias">
          {snapshot.alerts.state !== "ready" ? <DataState kind={snapshot.alerts.state} title={snapshot.alerts.state === "loading" ? "Cargando alertas" : "No pudimos verificar las alertas"} /> : alerts.length ? <>
            {alerts.slice(0, 2).map(a => <div key={a.id} className={`hoy-alert hoy-alert-${a.severity}`}><span className="hoy-alert-symbol" aria-hidden="true">{a.severity === "critical" ? "!" : "◐"}</span><div className="min-w-0"><p className="text-sm font-semibold">{a.title}</p><p className="mt-1 text-xs text-muted hoy-wrap">{a.description}</p><p className="mt-2 text-xs text-muted">{momentLabel(a.at)}</p>{a.action && <a className="mt-2 inline-block text-xs text-meta underline" href={a.action.href}>{a.action.label}</a>}</div></div>)}
            {alerts.length > 2 && <details className="hoy-more"><summary>Ver {alerts.length - 2} avisos más</summary>{alerts.slice(2).map(a => <div key={a.id} className="my-3 text-sm hoy-wrap"><b>{a.title}</b> · {a.description}{a.action && <> · <a className="text-meta underline" href={a.action.href}>{a.action.label}</a></>}</div>)}</details>}
      ...
      <Card className="hoy-agent"><details id="hoy-agent"><summary className="cursor-pointer font-semibold">Estado de los datos</summary>
        <div className="hoy-agent-mode"><span className="hoy-orbit" aria-hidden="true">✳</span><div><p className="font-semibold">{agent.mode === "off" ? "Agente detenido" : agent.mode === "semi" ? "Revisión humana" : agent.mode === "auto" ? "Autonomía configurada" : "Estado sin verificar"}</p><p className="text-xs text-muted">{agent.execution === "simulation" ? "Las decisiones se ensayan, no se envían a Meta." : "Desde esta pantalla no se envían cambios a Meta."}</p></div></div>
        <dl className="hoy-facts"><div><dt>Datos actualizados</dt><dd>{momentLabel(agent.collectedAt)}<span className="block text-muted">{agent.collection === "ok" ? "Sincronización completa" : agent.collection === "error" ? "La última sincronización falló; se reintenta sola" : agent.collection === "running" ? "Sincronizando ahora" : "Sin verificar"}</span></dd></div><div><dt>Última revisión del agente</dt><dd>{momentLabel(agent.strategyAt)}</dd></div><div><dt>Freno del agente</dt><dd>{agent.brake === "engaged" ? "Activo · no pausa anuncios en Meta" : agent.brake === "released" ? "Liberado" : "Sin verificar"}</dd></div></dl>
      </details></Card>
    </div>
    <div className="hoy-secondary">
      ...
      <Card title="Últimos cambios y su resultado" eyebrow="BITÁCORA">
        {snapshot.activity.state !== "ready" ? <DataState kind={snapshot.activity.state} title="Actividad no disponible" /> : snapshot.activity.data.length ? <ul className="hoy-history">{snapshot.activity.data.slice(0, 5).map(item => <li key={item.id}><div className="hoy-history-top"><p className="font-semibold">{item.actor}</p><span className="text-xs text-muted">{momentLabel(item.at)}</span></div><p className="mt-2 text-sm text-muted hoy-wrap">{item.summary}</p>{item.result && <div className="mt-2"><Chip tone={item.result.tone}>{item.result.label}</Chip><p className="mt-1 text-xs text-muted">{item.result.detail}</p></div>}<a className="mt-2 inline-block text-sm text-meta underline" href={sessionHref(item.id, account.id, `/hoy?account=${account.id}`)}>Ver detalle →</a></li>)}</ul> : <p className="text-sm text-muted">No hay cambios de personas registrados en esta lectura.</p>}
      </Card>
    </div>
  </div>;
}
