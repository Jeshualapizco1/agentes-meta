...
export function HoyDashboard({ snapshot, accountControl, onReview, onControl }: {
  snapshot: HoySnapshot; accountControl?: ReactNode; onReview: (id: string) => void; onControl: () => void;
}) {
  ...
  return <div className="hoy-page">
    <header className="hoy-header"><div><p className="hoy-eyebrow">HOY · {shortDate(snapshot.reportingDate)}</p><h1>Lo importante, primero.</h1><p className="hoy-subtitle">Prioridades, evidencia y resultados en una sola vista.</p></div>{accountControl}</header>
    <div className="hoy-context"><div className="hoy-context-account"><span className="hoy-account-mark" aria-hidden="true">{account.name.slice(0, 1)}</span><div><p className="font-semibold hoy-wrap">{account.name}</p><p className="text-xs text-muted">{account.currency} · calendario de la cuenta: {account.timeZone}</p></div></div>
    ...
    <div className="hoy-period"><span>Ventana de 7 días cerrados · {model.period}</span><span>El día en curso queda fuera</span></div>
    <div className="hoy-kpis">
      ...
      <MetricCard label="Compras atribuidas" value={model.current.purchases} format={v => v.toLocaleString("es-MX")} period="Atribución de Meta · no ventas netas" dataState={model.state} previous={model.canCompare ? { value: model.previous.purchases, label: "7 días previos:" } : undefined} />
    ...
    <div className="hoy-bento">
      <Card hero className="hoy-attention" eyebrow="TU SIGUIENTE DECISIÓN" title="Necesita tu atención" action={<Chip tone="neutral">{snapshot.proposals.state === "ready" ? `${proposals.length} propuestas` : "Cola sin verificar"}</Chip>}>
        <section className="hoy-alerts" aria-label="Alertas prioritarias">
          ...
          </> : <p className="hoy-quiet-note">No hay alertas abiertas en esta lectura. Esto no verifica por sí solo la salud de la cuenta.</p>}
        </section>
        {snapshot.proposals.state !== "ready" ? <DataState kind={snapshot.proposals.state} title={snapshot.proposals.state === "loading" ? "Cargando propuestas" : "No pudimos cargar las propuestas"} /> : proposals.length ? <><ul className="hoy-proposals">{proposals.slice(0, 2).map(proposalRow)}</ul>{proposals.length > 2 && <details className="hoy-more"><summary>Ver {proposals.length - 2} propuestas más</summary><ul className="hoy-proposals">{proposals.slice(2).map(proposalRow)}</ul></details>}</> : <DataState kind="empty" title="No hay propuestas pendientes" description="Puedes revisar los últimos resultados y la actividad del agente. No hay nada que aprobar en esta lectura." action={<a href="#hoy-agent" className="ui-button ui-button-secondary">Ver actividad del agente</a>} />}
      </Card>
      ...
      <Card title="Actividad del agente" eyebrow="LECTURA Y CONTROL" className="hoy-agent"><div id="hoy-agent" tabIndex={-1}>
    ...
    <div className="hoy-secondary">
      ...
      <Card title="Cambios de tu equipo" eyebrow="CONTEXTO, NO CAUSALIDAD">
