...
3. **Estratega** (Fase 4, infraestructura construida 2026-09-04): corre en el collector una vez al día con el día anterior cerrado; genera candidatos desde `rules` (hoy solo las dos de techo, que son candados), evalúa los candados en fila (core, `evaluateLocks`) y registra **propuestas** en la cola de Hoy (modo semi: aprobar/rechazar con razón). Deja huella en la Bitácora como actor agente aunque no proponga. Las reglas de acción se transcriben de `docs/06` cuando Eduardo conteste; el generador de candidatos (`generateCandidates`) es el punto de extensión. En Fase 4b un **ejecutor** aplica lo aprobado con write-ahead log y rollback, y cada regla pasa a **auto** cuando acumula N propuestas aprobadas sin corrección; cualquier rechazo, corrección o rollback la regresa a semi. Freno de emergencia por cuenta.
...
- **Escritura en Meta (Fase 4b, en simulado):** el único módulo que escribe es `MetaWriter` (`packages/meta/src/write.ts`) con tres operaciones: pausar anuncio, cambiar presupuesto diario, mover presupuesto entre dos campañas como par atado. `validateOrder` (core) rechaza todo lo demás antes de llegar ahí. Write-ahead log en `executions` (registrada → enviada → confirmada | fallida): si no se registra, no se envía. `account_profiles.dry_run` está en `true` por defecto: aprobar una propuesta simula toda la tubería sin llamar a Meta y la deja como "simulada". Apagarlo es decisión del dueño. La web de Netlify no tiene token de Meta: las aprobadas desde ahí las ejecuta (simula) el collector en su siguiente pasada.
- **Alertas por Telegram:** solo críticas al momento, resumen diario tras la pasada del estratega, propuestas pendientes y expiradas; warning e info se quedan en /estado. Cada envío queda en `notifications` con clave única. Secretos `TELEGRAM_BOT_TOKEN` y `TELEGRAM_CHAT_ID` (docs/02).
...
- **Paginar siempre.** PostgREST (Supabase) devuelve como máximo 1,000 filas por petición aunque se pida `.limit(50000)`; el corte es silencioso. Toda lectura que pueda pasar de unas cientos de filas va por `fetchAll` (`packages/db` para agentes, `apps/web/lib/db.ts` para la web) y nunca por un `select` a secas. Este corte dejó 16 sesiones sin campaña, reexpresiones sin detectar y la vista Anuncios en 1,000 de ~3,000 anuncios (2026-09-04).
