...
- [ ] Al pedir `/bitacora` sin sesión, el destino tras el login es `/hoy` (`next=%2Fhoy`) en vez de la ruta pedida: el layout privado llama `requireUser("/hoy")` antes que la página. Menor; corregir para que el enlace profundo se respete.
