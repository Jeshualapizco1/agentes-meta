 ...
 *  - Lenguaje: "coincidió con", nunca "causó".
 */
...
export interface Evaluation {
  ...
  delta: { roas_pct: number | null; cpa_pct: number | null; spend_pct: number | null; control_roas_pct: number | null; control_cpa_pct: number | null; diff_roas_pts: number | null; diff_cpa_pts: number | null };
  confidence: Confidence; verdict: string;
}
...
export function evaluateChange(o: { changeDate: string; campaignIds: string[]; rows: DailyRow[]; horizon: Horizon; today: string }): Evaluation {
  ...
  let confidence: Confidence = status === "pending" || treatment.after.spend === 0 || treatment.before.spend === 0 ? "insufficient" : purchases >= 60 ? "high" : purchases >= 30 ? "medium" : purchases >= 10 ? "low" : "insufficient";
  ...
  return { horizon: o.horizon, status, starts_at: afterFrom, ends_at: afterToPlanned, treatment, control, delta: { roas_pct: r1(roas_pct), cpa_pct: r1(cpa_pct), spend_pct: r1(spend_pct), control_roas_pct: r1(control_roas_pct), control_cpa_pct: r1(control_cpa_pct), diff_roas_pts: r1(diff_roas_pts), diff_cpa_pts: r1(diff_cpa_pts) }, confidence, verdict };
}

const THRESHOLD_PTS = 10; // puntos porcentuales de ROAS frente al control para hablar de mejora o deterioro
...
export interface WeeklySession { id: string; started_at: string; actor_name: string; summary: string; resets_learning: boolean; kind: string; evaluations: Pick<Evaluation, "horizon" | "status" | "confidence" | "verdict" | "delta">[] }
export interface WeeklyEvidence {
  ...
  campaigns: { best: { id: string; name: string; spend: number; purchases: number; roas: number }[]; worst: { id: string; name: string; spend: number; purchases: number; roas: number }[] };
